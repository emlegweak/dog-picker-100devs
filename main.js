...
fetch(BREEDS_URL)
    ...
    .then(data => {
        ...
        for (let i = 0; i < breedsArray.length; i++){
            ...
            option.innerText = breedsArray[i]; //displays breed value <option value = 'breed'>breed</option>
            select.appendChild(option); //creating a list of options to be used within the select tag, adds current <option> tag to the select box list of options
        }
    })
